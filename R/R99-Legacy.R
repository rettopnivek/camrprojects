...
# PROJECT-Description-YYYY_MM_DD-HH_MM-git_version.ext



camr_path_to_file <- function(
    chr_partial_name,
    chr_env_variable = NULL,
    chr_folder = NULL,
    lgc_latest = TRUE,
    ... ) {

  # Check inputs
  checkmate::assert_string( chr_partial_name )
  checkmate::assert_logical( lgc_latest )

  # Initialize path to source folder
  path_to_source_folder <- ""

  # If environmental variable provided
  if ( !is.null( chr_env_variable ) ) {

    # Check input
    checkmate::assert_string( chr_env_variable )

    # Extract path to source folder
    path_to_source_folder <- Sys.getenv( chr_env_variable )
  ...
  if ( !is.null( chr_folder ) ) {

    # Check input
    checkmate::assert_string( chr_folder )

    # Use as path to source folder
    if ( path_to_source_folder == "" ) {

      path_to_source_folder <- chr_folder

      # Close 'Use as path to source folder'
    } else {

      extended_path <- grepl( "/", chr_folder, fixed = TRUE )
      ...
      if ( !extended_path ) {
        ...
        if ( !( chr_folder %in% all_files_and_folders ) ) {

          any_matches <-
            grepl( chr_folder, all_files_and_folders, fixed = TRUE ) &
          ...
          if ( any_matches ) {
            ...
            matching_folders <- all_files_and_folders[ any_matches ]
      ...
      # Close else for 'Use as path to source folder'
    }
  ...
  partial_matches <- grepl( chr_partial_name, all_files, fixed = T )
...
camr_load_object_from_rdata <- function( chr_path, chr_object ) {

  # Check inputs
  checkmate::assert_file( chr_path, extension = '.RData' )
  checkmate::assert_file_exists( chr_path )
  checkmate::assert_string( chr_object )

  # Load in objects
  load( chr_path )

  # If R object does not exist
  if ( !exists( chr_object ) ) {

    err_msg <- paste0(
      "File does not contain '",  chr_object, "'."
    )
    stop( err_msg )

    # Close 'If R object does not exist'
  }

  # Return specified object
  return( get( chr_object) )
}
...
camr_file_proj_desc_dt_commit_ext <- function(
    description,
    ext = '.RData',
    project = NULL,
    date_time = NULL,
  ...
  if ( is.null( project ) ) {
    ...
    project <- Sys.getenv( "PROJECT_LABEL" )
